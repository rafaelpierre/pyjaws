"""CLI Entrypoints for Workflows functionality."""

import importlib
import pathlib
import glob
import logging
import click
import os
import sys
from pyjaws.api import jobs
import ruamel.yaml
import json
from pathlib import Path

logging.basicConfig(level="DEBUG")


def create_workflow(script_path: str):
    module_path = script_path.split(".py")[0]
    module_path = module_path.replace("/", ".")
    logging.info(f"Parsing script {module_path}...")
    logging.info(f"CWD: {os.getcwd()}")
    sys.path.append(os.getcwd())
    module = importlib.import_module(module_path)

    workflow = module.workflow
    logging.info(f"Imported workflow: {workflow.json()}")

    return workflow


@click.group()
def app():
    click.echo("Main")
    pass


@click.command(name="create")
@click.argument("input_folder", type=str, default="./")
@click.option(
    "--overwrite",
    is_flag=True,
    show_default=True,
    default=False,
    help="Overwrite existing workflow.",
)
def create(input_folder: str, overwrite: bool = False):
    """
    Creates a Jobs Workflow for the Python job definitions stored in
    input_folder.

    Params:
        input_folder: Path to the input Python files.
    """

    try:
        logging.info("Starting workflow creation task")

        path = pathlib.Path(input_folder)
        target_path = f"{str(path)}/*.py"
        logging.info(f"Trying to parse all workflows from directory: {target_path}")
        target_scripts = [
            target for target in glob.glob(target_path) if "__init__.py" not in target
        ]
        logging.info(f"Found: {target_scripts}")

        for script_path in [
            target for target in target_scripts if "__init__.py" not in target
        ]:
            workflow = create_workflow(script_path)
            logging.info(f"Deploying job workflow for {script_path}...")
            jobs.create(workflow, overwrite)

        logging.info("Finished workflow creation task")
        return sys.exit(0)

    except Exception as exception:
        logging.error(f"Error deploying jobs: {str(exception)}")
        raise exception


@click.command(name="create_config")
@click.argument("input_folder", type=str, default="./")
@click.option(
    "--file-format",
    type=click.Choice(["json", "yml"]),
    default="yml",
    help="Save the workflow as json or yml.",
)
def create_config(input_folder: str, file_format: str = "yml"):
    """
    Creates a configuration file that can be used to deploy a Workflow in dbx.

    Params:
        input_folder: Path to the input Python files.
        format: Format of the configuration file. Can be json or yaml.
    """

    try:
        logging.info("Starting workflow creation task")

        path = pathlib.Path(input_folder)
        target_path = f"{str(path)}/*.py"
        logging.info(f"Trying to parse all workflows from directory: {target_path}")
        target_scripts = [
            target for target in glob.glob(target_path) if "__init__.py" not in target
        ]
        logging.info(f"Found: {target_scripts}")

        for script_path in [
            target for target in target_scripts if "__init__.py" not in target
        ]:
            workflow = create_workflow(script_path)
            logging.info(f"Saving configuration file at {input_folder}/config/")

            workflow = create_workflow(script_path)
            config_folder = Path(input_folder) / "config"
            config_folder.mkdir(parents=True, exist_ok=True)

            if file_format == "json":
                config_file = config_folder / f"{workflow.name}.json"
                with config_file.open("w") as f:
                    json.dump(workflow.json(), f)
            elif file_format == "yml":
                config_file = config_folder / f"{workflow.name}.yml"
                with config_file.open("w") as f:
                    f.write(ruamel.yaml.round_trip_dump(workflow.json(), indent=2))
            else:
                raise Exception(f"Invalid format: {file_format}")

        logging.info("Finished saving workflow configuration files")
        return sys.exit(0)

    except Exception as exception:
        logging.error(f"Error deploying jobs: {str(exception)}")
        raise exception


app.add_command(name="create", cmd=create)
app.add_command(name="create-config", cmd=create_config)


def main():
    app(standalone_mode=False)
